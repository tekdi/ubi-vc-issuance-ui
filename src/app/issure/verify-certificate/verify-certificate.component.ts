...
export class VerifyCertificateComponent implements OnInit {
  ...
  constructor(
    ...
    if (
      localStorage.getItem("entity") == null ||
      ...
      const sideMenu = this.renderer.selectRootElement(
        ".layout-container",
        true
      );
      if (sideMenu) {
  ...
  async scanHandler($event: any) {
    ...
    this.verifyVc(
      await this.verifyService.scanSuccessHandler($event, this.itemData),
      "scan"
    );
  }
    ...
    this.itemData = {
      ...
      showResult: [
        { title: "Candidate No", path: "credentialSubject.candidateNo" },
        {
          ...
          path: "credentialSubject.certificateNo",
          removeStr: "did:abha:",
        },
        {
          title: "Name",
      ...
      scanNote:
        "To verify pledge certificate, simply scan the QR code thats on the document.",
  ...
  doVerify() {
    ...
    this.generalService
      .postData(environment.baseUrl + "/registry/api/v1/Results/search", body)
      .subscribe((res) => {
  ...
  verifyVc(uniqueIdorUrl, type) {
    ...
    this.generalService.getData(url, true, null, isMiddleUrl).subscribe(
      (res) => {
        if (res.status === "ISSUED") {
          let vcUrl = "credentials/credentials/" + uniqueId;
          this.generalService.getData(vcUrl, true, null, true).subscribe(
            (res) => {
              console.log({ res });
