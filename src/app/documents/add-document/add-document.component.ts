...
export class AddDocumentComponent implements OnInit {
  ...
  property: {
    name: { type: string; title: string };
    fileUrl: {
      type: string;
      title: string;
      widget: { formlyConfig: { type: string } };
    };
  };
  width: any = "100";
  imageFile: File;
  ...
  dataURItoBlob(dataURI: string): Observable<Blob> {
    return Observable.create((observer: Observer<Blob>) => {
      const byteString: string = window.atob(
  ...
  createStaticForm() {
    if (this.docType === "scan") {
      let schema: any = {
        name: {
          title: this.generalService.translateString("NAME_OF_DOCUMENT"),
          type: "string",
        },
      };
      ...
      let schema: any = {
        ...
        fileUrl: {
          title: this.generalService.translateString("DOCUMENT"),
          type: "string",
        },
      };
      ...
      this.schema["properties"]["name"]["widget"]["formlyConfig"] = {};
      this.schema["properties"]["fileUrl"]["widget"]["formlyConfig"] = {
        type: "file",
      };
      this.schema["properties"]["name"]["widget"]["formlyConfig"][
        "templateOptions"
    ...
    this.generalService.postData("/Issuer/search", search).subscribe(
      (res) => {
        ...
        if (this.doc_data["steps"] && this.doc_data["steps"].length > 0) {
          ...
          this.schema_property = JSON.parse(
            this.doc_data["steps"][this.step]["form"]
          );
        } else {
  ...
  submit() {
    if (this.steps_length > 0 && this.steps_length != this.step) {
      let api = this.doc_data["steps"][this.step]["api"];
      ...
      this.generalService.postData(api, this.model).subscribe((res) => {
        ...
        if (this.steps_length == this.step) {
          ...
          for (let [key, value] of Object.entries(this.schema_property)) {
            ...
            if (value["value"]) {
              let datavalue = this.getValue(value["value"], this.doc_data);
              ...
              if (this.schema_property[key]["hidden"]) {
                ...
                this.schema_property[key]["widget"]["formlyConfig"][
                  "templateOptions"
                ]["type"] = "hidden";
              }
            }
      ...
      if (!this.osid) {
        ...
        if (this.docType === "scan") {
          ...
          this.generalService.getData(this.entity).subscribe((res) => {
            let url = [this.entity, res[0]["osid"], "attestation", "documents"];
            this.generalService
              .postData(url.join("/"), formData)
              .subscribe((res2) => {
  ...
  getValue(value, steps) {
    ...
    data.forEach((element) => {
      let num = this.isNumeric(element);
      if (num) {
        try {
  ...
  postData(url, data) {
    ...
    this.generalService.postData(url, data).subscribe(
      (res) => {
        ...
        this.mySubscription = interval(5000).subscribe((x) => {
          ...
          if (x == 12) {
            ...
            this.toastMsg.error(
              "error",
              "Taking more time than usual to fetch document from issuer, something went wrong, please try again"
            );
  ...
  getPublishedData() {
    this.generalService.getData(this.entity).subscribe((res) => {
      console.log("res", res);
      let document = res[0][this.policyName].filter((doc) => {
