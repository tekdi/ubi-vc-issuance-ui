...
export class FormsComponent implements OnInit {
    ...
    this.schemaService.getFormJSON().subscribe(
      (FormSchemas) => {
        ...
        if (this.formSchema.description) {
          this.headingDescription = this.translate.instant(
        ...
        this.schemaService.getSchemas().subscribe(
          (res) => {
            ...
            this.formSchema.fieldsets.forEach((fieldset) => {
              ...
              if (
                this.formSchema.hasOwnProperty("wrapper") &&
                this.formSchema.wrapper == "stepper"
              ) {
                if (!this.schema.hasOwnProperty("widget")) {
                  this.schema["widget"] = {};
                }
    ...
    if (field.children.fields && field.children.fields.length > 0) {
      ...
      if (field.children.formclass) {
        this.responseData.definitions[fieldset.definition].properties[
          ...
          fieldGroupClassName: field.children.formclass,
        };
      }
      ...
      fieldset.fields.forEach((field) => {
        ...
        if (field.custom && field.element) {
          ...
          if (field.hasOwnProperty("validators")) {
            ...
            this.responseData.definitions[fieldset.definition].properties[
              field.name
            ]["widget"]["validators"] = field.hasOwnProperty("validators");
            if (tempObj != undefined) {
              tempObj["hideExpression"] =
                this.responseData.definitions[fieldset.definition].properties[
                  field.name
                ]["widget"]["formlyConfig"]["hideExpression"];
              this.responseData.definitions[fieldset.definition].properties[
                field.name
              ]["widget"]["formlyConfig"] = tempObj;
            }
          }
        } else {
  ...
  async postData() {
    ...
    let useMiddleUrl =
      this.formSchema.hasOwnProperty("useMiddleUrl") &&
      this.formSchema.useMiddleUrl
        ? this.formSchema.useMiddleUrl
        : false;
    this.model["sorder"] = this.exLength;
    await this.generalService
      .postData(this.apiUrl, this.model, false, useMiddleUrl)
      .subscribe(
        (res) => {
