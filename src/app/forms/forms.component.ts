...
exLength : number = 0
  ...
  constructor(private route: ActivatedRoute,
    ...
    public toastMsg: ToastMessageService, public router: Router, public schemaService: SchemaService, private formlyJsonschema: FormlyJsonschema, public generalService: GeneralService, 
    private location: Location, private successModalService: SuccessModalService) { }
  ...
  ngOnInit(): void {
    ...
    this.schemaService.getFormJSON().subscribe((FormSchemas) => {
      ...
      if (this.formSchema.description) {
        this.headingDescription = this.translate.instant(this.formSchema.description);
      }
      ...
      fieldset.fields.forEach(field => {
        ...
        if (field.custom && field.element) {
          ...
          if (field.hasOwnProperty('validators')) {
              ...
              this.responseData.definitions[fieldset.definition].properties[field.name]['widget']['validators'] = field.hasOwnProperty('validators');
...
let entity = this.entityName.charAt(0).toUpperCase() + this.entityName.slice(1);
  ...
  async postData() {
    ...
    let useMiddleUrl = (this.formSchema.hasOwnProperty('useMiddleUrl') && this.formSchema.useMiddleUrl) ? this.formSchema.useMiddleUrl : false;
    this.model['sorder']  = this.exLength;
    await this.generalService.postData(this.apiUrl, this.model, false, useMiddleUrl).subscribe((res) => {
      if (res.params.status == 'SUCCESSFUL' && !this.model['attest']) {
        if(this.formSchema['modal'])
        {
