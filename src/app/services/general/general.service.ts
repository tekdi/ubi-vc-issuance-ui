...
export class GeneralService {
  ...
  shareFile(fileType: string, url: string, typeRes: string, certNo: string) {
    ...
    const fileName = `${typeRes}_${localStorage.getItem(
      "loggedInUser"
  ...
  downloadFile(
    ...
    switch (lowerFileType) {
      ...
      case "png":
        ...
        break;
      case "json":
        fileTypeHeader = "image/json";
        blobType = "image/json";
        fileExtension = "json";
        break;
      default:
        console.error("Unsupported file type requested");
        alert("Unsupported file type. Please choose PDF, JPEG, PNG or JSON.");
        return;
    }
  ...
  shareDocument(sharingType, url, templateId, typeRes) {
    ...
    let fileName = `${typeRes}_${localStorage.getItem(
      "loggedInUser"
    )}_${new Date().toISOString()}`;
    ...
    this.http.get(`${this.middleUrl}${url}`, requestOptions).subscribe(
      (data) => {
        ...
        switch (sharingType) {
          ...
          case "facebook":
            window.open(
              `https://www.facebook.com/sharer/sharer.php?u=${blobUrl}`,
              "_blank"
            );
            break;
          case "linkedin":
            window.open(
              `https://www.linkedin.com/sharing/share-offsite/?url=${blobUrl}`,
              "_blank"
            );
            break;
          case "sms":
