...
export class SubheaderComponent implements OnInit {
  ...
  years: string[] = [];
  classTypes = [
    { title: "Upper Basic", value: "bcece" },
    { title: "Middle Basic", value: "middle basic" },
  ];
  ...
  buildsubMenu() {
    if (
      this.menuConfig?.subMenus?.left[0]?.redirectTo &&
      this.menuConfig?.subMenus?.left[0]?.redirectTo.includes(":")
    ) {
      ...
      urlParam.forEach((paramVal, index) => {
        if (paramVal.startsWith(":")) {
          ...
          if (
            key === "id" &&
            (this.jssidValue || localStorage.getItem("jssid"))
          ) {
            urlParam[index] = this.jssidValue
              ? `${this.jssidValue}`
              : localStorage.getItem("jssid"); // Replace with actual jssid
          }
  ...
  checkSubHeaderTitle() {
    if (
      this.menuConfig.hasOwnProperty("subHeaderTitleNotshow") &&
      !this.menuConfig.subHeaderTitleNotshow.some((path) =>
        this.location.path().includes(path)
      )
    ) {
  ...
  isPathAvailable() {
    if (
      this.menuConfig.hasOwnProperty("subMenus") &&
      this.menuConfig.subMenus.hasOwnProperty("left") &&
      this.menuConfig.subMenus.left.length
    ) {
      const currentPath = this.location.path();
      for (let i = 0; i < this.menuConfig.subMenus.left.length; i++) {
        if (
          this.menuConfig.subMenus.right[i].hasOwnProperty(
            "iscorrectPathToshowBtn"
          ) &&
          this.menuConfig.subMenus.left[i].iscorrectPathToshowBtn.some((path) =>
            currentPath.includes(path)
          )
        ) {
          console.log(this.isDataAvailable);
          this.menuConfig.subMenus.left[i]["showButton"] = true;
        } else this.menuConfig.subMenus.left[i]["showButton"] = false;
      }
    }
    if (
      this.menuConfig.hasOwnProperty("subMenus") &&
      this.menuConfig.subMenus.hasOwnProperty("right") &&
      this.menuConfig.subMenus.right.length
    ) {
      const currentPath = this.location.path();
      for (let i = 0; i < this.menuConfig.subMenus.right.length; i++) {
        if (
          ...
          this.menuConfig.subMenus.right[i].iscorrectPathToshowBtn.some(
            (path) => currentPath.includes(path)
          )
        ) {
          this.menuConfig.subMenus.right[i]["showButton"] = true;
        } else this.menuConfig.subMenus.right[i]["showButton"] = false;
      }
    }
  ...
  onClassChange(event: any): void {
    ...
    let paramf = [
      { value: this.selectedYear, filter: "academicYear" },
      { value: this.selectedClassType, filter: "schoolType" },
    ]; // this.sharedDataService.setSelectedAcademicYear(selectedValue)
    this.filterChanged(paramf);
  }
