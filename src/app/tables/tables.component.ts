...
export class TablesComponent implements OnInit {
  ...
  url: string =
    "https://sunbirdrc-sandbox.xiv.in/public/dashboards/N6Rl6lfgdvfflB3sJLLQJtqp6ZSrD23TSSj9a35U?org_slug=default";
  ...
  async postData() {
    ...
    this.property = [];
    if (
      this.tableSchema.hasOwnProperty("callMultiApi") &&
      this.tableSchema["callMultiApi"]
    ) {
  ...
  async addDataMultiCall() {
    ...
    if (this.tableSchema.getFromStoreVal) {
      await this.replacePlaceholders(
        this.tableSchema.body,
        "$" + this.tableSchema.getFromStoreVal,
        localStorage.getItem(this.tableSchema.getFromStoreVal)
      );
    }

    this.model = [];
    this.generalService
      .postData(get_url, this.tableSchema.body)
      .subscribe((res) => {
        if (res.hasOwnProperty("content")) {
          ...
          this.tableSchema.fields.forEach((field) => {
            ...
            if (temp_object.custom) {
              if (temp_object.type == "button" && !temp_object.isMultiple) {
                if (
                  temp_object.redirectTo &&
                  temp_object.redirectTo.includes(":")
                ) {
                  ...
                  temp_object["redirectToUrl"] = JSON.stringify(
                    urlParam.join("/").replace("//", "/")
                  );
                } else {
                ...
                if (temp_object.type == "button" && temp_object.isMultiple) {
                  for (let i = 0; i < temp_object.buttons.length; i++) {
                    if (
                      temp_object.buttons[i].redirectTo &&
                      temp_object.buttons[i].redirectTo.includes(":")
                    ) {
                      let urlParam =
                        temp_object.buttons[i].redirectTo.split(":");
                      urlParam.forEach((paramVal, index) => {
                        if (paramVal in element) {
                          urlParam[index] = element[paramVal];
                        }
                      });
                      temp_object.buttons[i]["redirectToUrl"] = urlParam
                        .join("/")
                        .replace("//", "/");
                    } else {
                      temp_object.buttons[i]["redirectToUrl"] =
                        temp_object.buttons[i].redirectTo;
                    }
                  }
                }
          ...
          if (temp_object.custom) {
            if (temp_object.type == "button" && !temp_object.isMultiple) {
              if (
                ...
                temp_object["redirectToUrl"] = urlParam
                  .join("/")
                  .replace("//", "/");
              } else {
                temp_object["redirectToUrl"] = temp_object.redirectTo;
  ...
  async actionFunction(action, data, proprties, index) {
    ...
    } else if (action == "post" && data["bodyVal"] == "*") {
    ...
    this.generalService
      .postData(data.api, params, true, isMiddleUrl)
      .subscribe((res) => {
        console.log({ res });
        ...
        if (data["successModal"]) {
          ...
          if (data["modal"].title.includes("$" + data.modal["bodyVal"])) {
            ...
            data["modal"].title = data["modal"].title.replace(
              "$" + data.modal["bodyVal"],
              selectItem
            );
          }
  ...
  downloadFile(filepath, item, typeRes, shouldUpload) {
    this.generalService.downloadFile(
      ...
      typeRes,
      item["res"]["certificateNo"],
      shouldUpload
    );
  }

  shareOnSocial(filepath, item, typeRes): void {
    this.generalService.shareFile(
      filepath,
      "/api/inspector/credentials",
      typeRes,
      item["res"]["certificateNo"]
